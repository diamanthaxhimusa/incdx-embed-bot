...
(function (root, factory) {
  ...
  incdxBot.init = function(options) {
    ...
    var optionsToggler = document.getElementById("incdx-bot_header-options-toggler");
    optionsToggler.addEventListener("click", toggleBotOptions);
    ...
    incdxRequest(urlToIncdxBot, {})
    .then((data) => {
      console.log('data', data)
      sessionId = data['session-id'];
      generateBotMessage(data.data);
    })
  ...
  socket.on('chat', function(data){
    if (data.sender != "diamant") {
      generateBotMessage(data, true);
    }
  });
  ...
  var toggleBotOptions = function() {
    var popup = document.getElementById("incdx-bot_header-options-popup");
  ...
  var sendMessage = function (message) {
    ...
    incdxRequest(urlToIncdxBot, {"query": message, "sessionId": sessionId})
      .then((data) => {
        console.log(data)
        if(loadingMessage) loadingMessage.remove();
        // Call generateBotMessage
        generateBotMessage(data, false);
      })
      .catch(error => {
        console.log(error)
        if(loadingMessage) loadingMessage.remove();
  ...
  var generateBotMessage = function(data, isAgent) {
    var resultWrapper = document.getElementById("resultWrapper");
    // create bot mesasage content and display it to chat
    ...
    if (!isAgent) {
      ...
      var nodeToAppend = generator["ioText"](data.message, botResultContainer);
      botMessage.appendChild(nodeToAppend);
    }
    resultWrapper.scrollTop = resultWrapper.scrollHeight;
  }
  ...
  */
  ...
  generator.ioText = (message) => {
    var textResponsesNode = document.createElement("div");
    textResponsesNode.classList.add("incdx-text-response");
    textResponsesNode.innerHTML = `<span>${message}</span>`;
    return textResponsesNode;
  }
   ...
   * @name suggestions
   * @param {Object} fulfillmentMessage Fulfillment Message from Dialogfow
   * @param {Node} botResultContainer Bot Result Container to add chip-container
   * @returns {Node} suggestionsNode
   */
  generator.suggestions = (fulfillmentMessage, botResultContainer) => {
    // botResultContainer.classList.add("chip-container");
    ...
    botResultContainer.appendChild(suggestionsNode);
   ...
   */
  generator.linkOutSuggestion = (fulfillmentMessage, botResultContainer) => {
    ...
    if (incdxSuggestionsChips.length) {
      ...
      botResultContainer.appendChild(linkOutSuggestionNode);
    }
  }
   ...
   */
  generator.carouselSelect = (fulfillmentMessage, botResultContainer) => {
    ...
    botCarouselMessage.appendChild(carouselSelectNode);
